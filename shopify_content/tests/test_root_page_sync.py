...
class SyncShopifyRootPageTests(TestCase):
    ...
    @patch('metaobjects.shopify_metaobjects.client.MetaobjectClient')
    def test_sync_mirror_for_unregistered_root(self, mock_client_cls):
        mock_client = MagicMock()
        mock_client.sync.return_value = Metaobject(
            type='root_page',
            handle='products',
        ...
        self.assertEqual(data['handle'], 'products')
        self.assertEqual(data['slug'], 'products-root')
        self.assertNotIn('locale', data)
