...
These consumers are pure payload builders: they know how to compute a precomputed
index JSON and its associated hreflang/noindex contract for a given locale, but they
never talk to Shopify directly. The single writer to Shopify is
`shopify_content.sync.outbound.sync_root_index_locales`, which upserts one `root_page`
metaobject entry per configured locale (see docs/shopify_content.md).
"""
...
class RootIndexConsumer:
    ...
    def build_alternates_payload(self, config: dict, all_locales: list[str]) -> dict:
        """
        Build the index_alternates JSON shared by every sibling root_page entry: the
        reciprocal hreflang/link-to-self-locales contract described in
        wagtail-root-index-section.plan.md, generic across any RootIndexConsumer.
